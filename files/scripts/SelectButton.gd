...
func create_select_button(array):
	...
	for i in array:
		...
		if i.has('bigtooltip'):
			checkbox.connect('mouse_entered', self, 'tooltipshow', [checkbox])
			checkbox.connect('mouse_exited', self, 'tooltiphide')
...
func _on_SelectButton_pressed():
	...
	tooltiphide()
	var pos = get_node("Popup").get_parent().rect_global_position
	pos.y = pos.y + get_node("Popup").get_parent().get_size().y
	get_node("Popup").rect_global_position = pos
