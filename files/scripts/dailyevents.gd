...
func dictionary():
	showntext = person.dictionary(showntext)
...
func showevent():
	...
	if person.imageportait != null:
			if File.new().file_exists(person.imageportait) == true:
...
func prositutebuyout(stage = 0):
	...
	if stage == 0:
		...
		showntext += "[color=yellow]You are offered " + str(round(price)) + person.dictionary(" gold for $name. [/color]")
	...
	if stage == 1:
		...
		if person.loyal < 25:
			showntext += person.dictionary("After some consideration $he agrees and you send them away.")
			globals.slaves.remove(globals.slaves.find(person))
			globals.resources.gold += price
		else:
			showntext += person.dictionary("$He hastily rejects this offer and declares $he can only acknowledge you as $his master.")
			person.loyal += rand_range(10,15)
	buttons = tempbuttons
	showevent()
...
func gift(stage = 0):
	...
	if stage == 1:
		...
		if person.consent == false:
			person.obed += -rand_range(20,40)
			person.loyal += -rand_range(5,10)
			showntext += person.dictionary("$name is disgusted by your implications and leaves infuriated. ")
		else:
			person.lastsexday = globals.resources.day
			showntext += person.dictionary("You spread your legs and bare your crotch, inviting $name over to which $he readily responds. After couple pleasant minutes of $name's eager mouth work you pass $him the requested money and return to your work. ")
			person.obed += rand_range(10,25)
...
func teenagersflirt(stage = 0):
	...
	if stage == 1:
		...
		if (person.sexuals.actions.has('pussy') || person.sexuals.actions.has('ass') ) && person.traits.has("Monogamous") == false:
			...
			person.loyal += -rand_range(5,10)
			showntext += person.dictionary("Getting caught up in the action, $name gets on all fours and lets one of the boys take $him from behind. ")
			...
			showntext += person.dictionary("$name greatly distressed with situation but having no ways out $he only keeps grudge against you. ")
			person.stress += rand_range(15,25)
			person.loyal += -rand_range(10,20)
			person.obed += -rand_range(30,50)
		showntext += person.dictionary("After short time the boys shower the $child in semen. Satisfied with your hospitality, they leave happy. ")
		globals.resources.mana += rand_range(3,6)
		if person.race == "Drow":
			globals.resources.mana += 1
	elif stage == 3:
